/*Write a program to draw two half circles x-axis wise and then draw its diameters.*/
...
void halfcircle_draw(int xc,int yc,int r,int c)
{
  ...
  //to draw circle
	for(i=0;i<=na;i++)
	{
	 ...
	 for(i=na+1;i<=n1;i++)
	{
	 putpixel(320+xc+x[i]-2*r,240-yc+y[i],c);
	}
	for(i=0;i<=4*r;i++)
	putpixel(320+xc+x[n3]-i,240-yc+y[n3],c);
}