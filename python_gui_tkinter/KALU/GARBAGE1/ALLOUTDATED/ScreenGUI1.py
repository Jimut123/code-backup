...
class GUIfunctions:
	def insert():
		...
		Button(parent1, text='Show', command=CommandsGUI.show_entry_fields).pack(side=LEFT, padx=5, pady=5)
		Button(parent1, text='Insert to database', command=DBOperations.insert_into_db).pack(side=LEFT, padx=5, pady=5)
		Button(parent1, text='Reset', command=Generic.combine_funcs(DBOperations.reset_val)).pack(side=RIGHT, padx=5, pady=5)
