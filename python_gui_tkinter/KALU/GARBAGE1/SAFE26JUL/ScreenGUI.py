...
class DataValidation:
	...
	def update_rows(tuple_rows):
		...
		Button(parent6, justify=tk.LEFT, padx =10, pady = 10, 
					text='show data', command=get_rows
		...
		for data in tuple_rows:
			...
			if data != "":
				data_str = str(data)
				tk.Label(parent6, justify=tk.LEFT, padx =10, pady = 10, 
							text=data_str,font=font.Font(family='Helvetica', size=20, 
							weight='bold')).grid(row=row_no,column=1,sticky=W, pady=4)
				entry1[counter] = tk.Entry(parent6,width = 100)
				entry1.insert(END, data_str)
				entry1[counter].grid(row=row_no, column=1,sticky=W, pady=4)
				counter = counter + 1
				row_no = row_no + 2
		
		Button(parent6, justify=tk.LEFT, padx =10, pady = 10, 
					text='update'#, command=CommandsGUI.show_entry_fields
					).grid(row=20, 
					column=1, sticky=W, pady=4)
		'''
