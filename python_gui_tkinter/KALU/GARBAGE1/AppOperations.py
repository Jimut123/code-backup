...
class AppOperations:
	...
	def updateData(slno,name, e_mail, flat, tower,  area, parking, recpt_fees, addr, contact_no,tmestmp):
		...
		conn.commit()
	def delete_selected(slno,name, e_mail, flat, tower,  area, parking, recpt_fees, addr, contact_no,tmestmp):
		print("values to be deleted : ",slno," ",name," ",e_mail," ", tower, flat, area, parking, recpt_fees, addr, contact_no,tmestmp)
		
		cur.execute(''' DELETE FROM details WHERE  sl_no = ? AND name = ? AND e_mail = ? AND tower = ? AND flat = ? AND area = ? AND parking = ? AND 
			recpt_fees = ? AND addr = ? AND contact_no = ? AND timestmp = ? ''',(slno,name, e_mail, tower, flat, area, parking, recpt_fees, addr, contact_no,tmestmp))
		conn.commit()
