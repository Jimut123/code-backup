/*Write a program to draw a half circle y-axis wise and then draw its diameter.*/
...
void halfcircle_draw(int xc,int yc,int r,int c)
{
  ...
  setcolor(2);
  sprintf(a,"D1(%d, %d)",xc+x[0],yc+y[0]);
  outtextxy(320+xc+x[0],240-yc+y[0],a);
  setcolor(2);
  sprintf(a,"D2(%d, %d)",xc+x[n1],yc+y[n1]);
  outtextxy(320+xc+x[n1],240-yc+y[n1],a);
  n=bresenham(x[0],y[0],x[n1],y[n1],xx,yy);
  for(i=0;i<=n;i++)
  {
