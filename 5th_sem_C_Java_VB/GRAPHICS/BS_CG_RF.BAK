...
Take refection along y-axis. Draw modified triangle.
...
int main()
{
	...
	/* To draw the original triangle */
	draw_triangle(x1, y1, x2, y2, x3, y3);

	/* To draw the triangle after reflection on x-axis */
	draw_triangle(x1, -y1, x2, -y2, x3, -y3);

	/* To draw the triangle after reflection on y-axis */
	draw_triangle(-x1, y1, -x2, y2, -x3, y3);

	/* To draw the triangle after reflection on y=x */
...
void draw_triangle(int x1, int y1, int x2, int y2, int x3, int y3)
{
	...
	/* To plot Line AB */
	outtextxy((320 + x[0]), (240 - y[0]), "B");
	for(i = 0; i < n; i++)
	{
		putpixel((320 + x[i]), (240 - y[i]), c_line);
		delay(DELAY);
	}
	outtextxy((320 + x[n - 1]), (240 - y[n - 1]), "B");outtextxy((320 + x[0]), (240 - y[0]), "A");
