	/* ancg_27.c: Write a program to draw a log(x) curve from 1 to
	to x*/
	...
	void log_x(float x,float y );
	void main()
	{
	float x1,x2;
	char a[100];
	clrscr();
	printf("\nEnter starting 'x' value of log(x) x1=");
	scanf("%f",&x1);
	printf("Enter ending 'x' value of log(x) x2=");
	scanf("%f",&x2);
	...
	/* void log_x(float x1, float x2) : Function to draw a
	log(x) curve */
	void log_x(float x1,float x2)
	{
	float x,y;
	int xx1,yy1;
		for(x=x1;x<=x2;x=x+1)
