	/*ancg_4.c: Write a program to draw a rectangle*/
	#include<stdio.h>
	#include<graphics.h>
	int n,c;
	void draw_xy();
void rectangle_draw(int x1,int y1,int x2,int y2,int x3,int y3,int x4,int y4);
	...
	scanf("%d",&y3);
	printf("Enter x-point of point-D=");
	scanf("%d",&x4);
	printf("Enter y-point of point-D=");
	scanf("%d",&y4);
...
void rectangle_draw(int x1,int y1,int x2,int y2,int x3,int y3,int x4,int y4)
	...
	line_draw(x,y,n,c);
	dda(x3,y3,x4,y4,x,y); /* To draw line CD */
	line_draw(x,y,n,c);
	n=0;
	dda(x4,y4,x1,y1,x,y); /* To draw line DA */
	outtextxy(250,200,a);
	line_draw(x,y,n,c);
	}
