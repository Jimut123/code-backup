	/* suther2.c: Write a program to implement Clipping algorithm using
	...
	if(y2>yT)
	bit2[3]=1;
	/* To calculate bit wise and operation */
	n=0;
		for(i=0;i<4;i++)
		{
		n=bit1[i] & bit2[i];
	...
	b2=bit_code(x2,y2);
		if(b1==0 || b2==0)
		{
		outtextxy(50,300,"Clipping not possible. Press any key.");
		getch();
		return;
		}
	b3=bit_and(x1,y1,x2,y2);
		if(b3 != 0)
		{
                outtextxy(50,300,"Clipping not possible. Press any key.");
		getch();
