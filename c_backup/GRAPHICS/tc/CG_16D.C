	/* cg_16d.c: Write a program to draw two half circle y-axis wise
	  and then draw  its diameters.
	    */
	#include<stdio.h>
	#include<graphics.h>
	int c;
	...
	int x[700],y[700];
	clrscr();
	printf("\nEnter x-coordinate of Centre of circle(xc)=");
	scanf("%d",&xc);
	printf("Enter y-coordinate of Centre  of circle(yc)=");
	scanf("%d",&yc);
	printf("Enter Radius of your circle(r)=");
	scanf("%d",&r);
	...
	/* To draw 1st half circle y-axis wise */

		for(i=n3+10;i<=3*n3-10;i++)
		{
		putpixel(320+xc+x[i],240-yc+y[i],c);
		delay(2500);
		}

	/* To draw 2nd half circle y-axis wise */
		for(i=0;i<=n3-10;i++)
		{
                putpixel(320+xc+x[i],240-yc+y[i],c);
		delay(2500);
		}
		for(i=3*n3+10;i<=4*n3-1;i++)
		{
                putpixel(320+xc+x[i],240-yc+y[i],c);
		delay(2500);
		}

	/* To draw two diameters */

	/* To draw a line from (n3+10,n3+10) to (3*n3-10,3*n3-10) */

	bresenham(x[n3+10],y[n3+10],x[3*n3-10],y[3*n3-10],xx,yy);
		for(i=0;i<=n;i++)
