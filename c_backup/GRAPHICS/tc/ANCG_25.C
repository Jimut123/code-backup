	/* ancg_25.c: Write a program to draw a sin, cos, tan curve from
	-theta to +theta*/
	...
	void sine(float theta1,float theta2);
	void cosine(float theta1,float theta2);
	void tangent(float theta1,float theta2);
	...
	char a[100];
	clrscr();
	printf("\nEnter starting angle in degrees(theta1)=");
	scanf("%f",&theta1);
	printf("Enter ending angle in degrees(theta2)=");
	scanf("%f",&theta2);
	printf("\nEnter color code of line(1,2,3,4,5,15)=");
	scanf("%d",&c);
	draw_xy();
	sine(theta1,theta2);
	cosine(theta1,theta2);
	tangent(theta1,theta2);
	/*sprintf(a,"C(%d,%d)",xc,yc);
	outtextxy(320+xc,240-yc,a); */
	getch();
	closegraph();
	}
	/* void sine(float theta1, float theta2) : Function to draw a
	sin(x) curve */
	void sine(float theta1,float theta2)
	{
	float x,y;
	int x1,y1;
		for(x=theta1;x<=theta2;x=x+1)
		{
		y=100*sin(x*3.141593/180);
		y1=y;
		x1=x/2;
	...
	/* void cosine(float theta1, float theta2) : Function to draw a
	cos(x) curve */
	void cosine(float theta1,float theta2)
	{
	float x,y;
	int x1,y1;
		for(x=theta1;x<=theta2;x=x+1)
		{
		y=100*cos(x*3.141593/180);
		y1=y;
		x1=x/2;
		putpixel(320+x1,240-y1,c+1);
		delay(2500);
		}
	}


	/* void tangent(float theta1, float theta2) : Function to draw a
	tan(x) curve */
	void tangent(float theta1,float theta2)
	{
	float x,y;
	int x1,y1;
		for(x=theta1;x<=theta2;x=x+1)
		{
		y=50*tan(x*3.141593/180);
		y1=y;
		x1=x/2;
		putpixel(320+x1,240-y1,c+3);
		delay(2500);
		}
	}
