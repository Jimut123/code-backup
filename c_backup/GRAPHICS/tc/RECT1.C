	/* rect1.c: Write a program to draw a rectangle ABCD
	where the points A(x1,y1), B(x2,y2), C(x3,y3) D(x4,y4) are given.
...
void rectangle_draw(int x1,int y1,int x2,int y2,int x3,int y3,int x4,int y4);
	void main()
	{
	int x1,y1,x2,y2,x3,y3, x4, y4,x[700],y[700];
	clrscr();
	...
	scanf("%d",&y3);
	printf("Enter x-coordinate of point D=");
	scanf("%d",&x4);
	printf("Enter y-coordinate of point D=");
	scanf("%d",&y4);
	...
	/* void rectangle_draw(int x1,int y1,int x2,int x3,int y3,int x4,int y4) :
	Function to draw a rectangle ABCD */
...
void rectangle_draw(int x1,int y1,int x2,int y2,int x3,int y3,int x4,int y4)
	...
	line_draw(x,y,n,3);
	dda(x3,y3,x4,y4,x,y); /* To generate points of line CD */
	line_draw(x,y,n,4);
	dda(x4,y4,x1,y1,x,y); /* To generate points of line DA */
	line_draw(x,y,n,4);
	/* To draw 2 diagonals */
	dda(x1,y1,x3,x3,x,y);
	line_draw(x,y,n,1);
